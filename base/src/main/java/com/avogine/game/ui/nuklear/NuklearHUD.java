...
public class NuklearHUD {
	...
	private int width = 1920, height = 1080; // The pixel dimensions of the GLFW window
	private int display_width = 1920, display_height = 1080; // The pixel dimensions of the content inside the window, this will usually be the same size as the window.

	private NkContext context = NkContext.create(); // Create a Nuklear context, it is used everywhere.
...
//		sidebar = new Sidebar(context, "Sidebar", 200, true);
	}
	...
	private NkContext setupWindow(long win) {
		...
		glfwSetCharCallback(win, (window, codepoint) -> nk_input_unicode(context, codepoint));
...
//		glfwSetCursorPosCallback(win, (window, xpos, ypos) -> nk_input_motion(context, (int)xpos, (int)ypos));
...
//		nk_input_end(context);
		...
		NkColor background = NkColor.create()
				.r((byte) 0).g((byte) 0).b((byte) 0).a((byte) 0);
		nk_style_push_color(context, style.window().background(), background);
		
		NkRect rect = NkRect.create().x(50).y(50).w(100).h(50);
		if (nk_begin(context, "HUD", rect, 0)) {
			// TODO Render the shots fired
			float rowHeight = 50;
			int itemsPerRow = 1;
